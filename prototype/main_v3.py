...
def safe_retry(fn, tries=3, sleep_sec=2, name="call"):
    last = None
...
def run():
    ...
    force_one_trade = os.getenv("FORCE_ONE_TRADE", "0") == "1"
    forced_done = False

    trace_id = new_trace_id()
    logger.log("BOOT", trace_id, {"msg": "prototype v3 starting", "force_one_trade": force_one_trade})
    ...
    while True:
        ...
        # candles fetch with retry
        try:
            spot_df = safe_retry(lambda: broker.get_spot_candles_1h(), tries=3, sleep_sec=2, name="spot_candles")
        except Exception as e:
        ...
        should_trade = (cond1 and cond2 and cond3 and cond4)

        if force_one_trade and (not forced_done):
            logger.log("FORCE_TRADE", trace_id, {"msg": "forcing 1 paper trade for validation"})
            should_trade = True
        ...
        if should_trade:
            ...
            logger.log("ENTRY_FILLED", trace_id, {"trade_id": trade_id, "fut": fut_symbol, "entry": fut_ltp, "hard_sl": hard_sl})

            # quick monitor loop for 15 seconds (prototype)
            start = time.time()
            fut_exit = fut_ltp
            reason = "TIMEBOX_EXIT"
            ...
            write_trade_csv("prototype/outputs/paper_trades.csv", TradeRow(
                ...
                regime="FORCED" if (force_one_trade and not forced_done) else "LIVE",
