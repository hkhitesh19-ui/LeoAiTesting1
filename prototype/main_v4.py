...
def run():
    ...
    logger.log("BOOT", trace_id, {"msg": "prototype v4 starting", "force_one_trade": force_one_trade})
    ...
    while True:
        ...
        # run indicators only when close changes
        if last_candle_close is not None and abs(close - last_candle_close) < 1e-9:
        ...
        if force_one_trade and (not forced_done):
            logger.log("FORCE_TRADE", trace_id, {"msg": "forcing 1 trade for validation"})
            should_trade = True
        ...
        logger.log("ENTRY_FILLED", trace_id, {"trade_id": trade_id, "fut": fut_symbol, "entry": fut_ltp, "hard_sl": hard_sl, "vix": vix})
