...
def next_hour_candle_open_ist():
    ...
    Execution: Next candle open + 1 sec
    """
    now = ist_now()
    d = now.day
    ...
    if now.minute < 15:
        ...
        hour = now.hour + 1
        if hour == 24:
            target = now.replace(day=d, hour=23, minute=59, second=59, microsecond=0)
        else:
...
def run():
    ...
    paper = PaperBroker()
    api = ShoonyaAdapter(trace_id=new_trace_id())

    trace_id = new_trace_id()
    logger.log("BOOT", trace_id, {"msg": "prototype starting", "capital": cfg.capital})
    ...
    while True:
        ...
        # ---- 1) Signal check
        try:
            spot_df = api.get_spot_candles_1h()
        ...
        # ---- 2) Execute at next candle open + 1 sec
        if pending_entry and (not in_trade):
            ...
            basis = float(fut_ltp) - float(stored_spot_close)
            hard_sl = float(fut_ltp) - (cfg.sl_atr_mult * float(stored_atr))

            if vix < cfg.vix_low_threshold:
                regime = "RATIO_SPREAD"
            else:
                regime = "FUT_PUT_HEDGE"

            logger.log("ENTRY_DECISION", trace_id, {
                "vix": vix, "regime": regime, "fut_symbol": fut_symbol, "fut_entry": fut_ltp,
                "basis": basis, "atr_spot": stored_atr, "hard_sl": hard_sl
            })
            ...
            # paper execution
            if regime == "FUT_PUT_HEDGE":
                paper.place_market("ATM_PUT_SIM", "BUY", 1, ltp=100.0)
                paper.place_market(fut_symbol, "BUY", 65, ltp=fut_ltp)
            else:
                paper.place_market("CE_ITM_SELL_SIM", "SELL", 65, ltp=120.0)
                paper.place_market("CE_ATM_BUY_SIM", "BUY", 130, ltp=80.0)
            ...
            logger.log("ENTRY_FILLED", trace_id, {
                ...
                "hard_sl": hard_sl, "positions": [p.__dict__ for p in paper.get_positions()]
            })

        # ---- 3) Monitor intrabar SL
        if in_trade:
            try:
                _, fut_ltp = api.get_future_ltp()
            ...
            if hard_sl is not None and fut_ltp <= hard_sl:
                ...
                for pos in paper.get_positions():
                    ...
                    paper.place_market(pos.symbol, side, abs(pos.qty), ltp=fut_ltp)
                ...
                write_trade_csv("prototype/outputs/paper_trades.csv", TradeRow(
                    ...
                    regime=regime,
                    spot_entry_ref=float(stored_spot_close),
                    fut_entry=float(fut_entry),
                    basis=float(fut_entry - stored_spot_close),
                    atr_spot=float(stored_atr),
